...
//            def result = gridConfig.callGridPropertyMethod('gridImplService', 'htmlGridDefinition')
    ...
    def gridData(GridConfig gridConfig) {
        ...
        log.trace "before calling appropriate transformation on grid definition...."
        log.trace "Grid Config: ${gridConfig}"
        log.trace "Rows: ${rows}"
        log.trace "NR Records: ${nrRecords}"
        log.trace "List Params: ${listParams}"
    ...
    GridConfig getGridConfig(controller, String gridName) {
        //log.trace "attempting to retrieve grid for controller: ${controller}, gridName: ${gridName}...."
        //log.trace "grid repository from which we look up....${gridRepository}"
        //log.trace "found grid config....${gridRepository[controller][gridName]}"
