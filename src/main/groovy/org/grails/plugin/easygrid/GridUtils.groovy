...
//            otherwise fallback to the slow implementation of Eval
    ...
    static PersistentProperty getPersistentProperty(Class domainClass, String propertyName) {
        resolvePersistentEntity(domainClass)?.getPropertyByName(propertyName)
    }

    protected static PersistentEntity resolvePersistentEntity(Class domainClass) {
        resolveDomainClassStaticApi(domainClass)?.gormPersistentEntity
    }

    protected static GormStaticApi resolveDomainClassStaticApi(Class domainClass) {
        GormEnhancer.findStaticApi(domainClass)
    }
